	...
	function qwery_trx_addons_addons_list( $list = array() ) {
		...
		if ( is_array( $list ) ) {
			...
			$required_addons = array(
                ...
                'smoke' => array( 'title' => esc_html__( 'Smoke / Fog', 'qwery' ), ),
                'bg-colors' => array( 'title' => esc_html__( 'Background colors', 'qwery' ), ),
				'expand-collapse' => array( 'title' => esc_html__( 'Expand / Collapse', 'qwery' ) ),
                'qw-extension' => array( 'title' => esc_html__( 'QW Extension', 'qwery' ), )
			);
...
if ( ! function_exists( 'qwery_trx_addons_thumb_size' ) ) {
	...
	function qwery_trx_addons_thumb_size( $thumb_size = '', $type = '', $args=array() ) {
        if ( empty($args['thumb_size']) && ('blogger-lay_portfolio' == $type && 'style-1' == $args['template_lay_portfolio'] || 'blogger-lay_portfolio' == $type && 'style-8' == $args['template_lay_portfolio'] )) {
			...
			$thumb_size = qwery_get_thumb_size('medium-square');
		}		
		return $thumb_size;
	}
}
...
if ( ! function_exists( 'qwery_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title', 'qwery_trx_addons_slider_title', 10, 4 );
	function qwery_trx_addons_slider_title( $title, $data, $args, $num ) {
		$title = '';
...
if ( ! function_exists( 'qwery_skin_add_trx_addons_options_mouse_helper' ) ) {
    add_filter( 'trx_addons_filter_options_mouse_helper', 'qwery_skin_add_trx_addons_options_mouse_helper' );
    function qwery_skin_add_trx_addons_options_mouse_helper( $args ) {
        $args['mouse_helper_in_swiper_slider'] = array(
            "title" => esc_html__('Show mouse helper in swiper slider', 'qwery'),
            "desc" => wp_kses_data( __('Display the mouse helper in swiper slider', 'qwery') ),
            "dependency" => array(
...
if ( ! function_exists( 'qwery_skin_mouse_helper_in_swiper_slider_localize_script' ) ) {
	add_action( 'trx_addons_filter_localize_script', 'qwery_skin_mouse_helper_in_swiper_slider_localize_script' );
	function qwery_skin_mouse_helper_in_swiper_slider_localize_script( $vars = array() ) {
