...
if ( apply_filters( 'qwery_filter_single_post_header', is_singular( 'post' ) || is_singular( 'attachment' ) ) ) {
	...
	ob_end_clean();
	$qwery_with_featured_image = qwery_is_with_featured_image( $qwery_post_header /*, array( 'with_gallery' )*/);
	// Post title and meta
	ob_start();
	...
	if ( strpos( $qwery_post_header, 'post_featured' ) !== false
		...
		<div class="post_header_wrap post_header_wrap_in_header post_header_wrap_style_<?php
            ...
            if ( $qwery_with_featured_image ) {
                echo ' with_featured_image' . ( false && qwery_get_theme_option( 'single_parallax' ) == 0 ? ' qwery-full-height' : '' );
            }
		?>">
			<?php
