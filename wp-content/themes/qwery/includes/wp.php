...
if ( ! function_exists( 'qwery_wp_theme_setup' ) ) {
	...
	function qwery_wp_theme_setup() {
		...
		// add_filter( 'qwery_filter_privacy_text', 'qwery_add_privacy_page_link' );
	}
}
...
if ( ! function_exists( 'qwery_get_wp_template_hooks' ) ) {
	/**
	 ...
	 */
	function qwery_get_wp_template_hooks() {
		return apply_filters( 'qwery_filter_wp_template_hooks', array(
					'404_template',
					'archive_template',
					'attachment_template',
					'author_template',
...
if ( ! function_exists( 'qwery_detect_blog_mode' ) ) {
	 ...
	 * 
	 * For example:
	 *   $pos = qwery_get_theme_option( 'sidebar_position' );
	 * 
...
if ( ! function_exists( 'qwery_get_current_mode_image' ) ) {
	...
	function qwery_get_current_mode_image( $default = '' ) {
		...
		return qwery_clear_thumb_size( $default );
	}
}
...
if ( ! function_exists( 'qwery_blog_archive_end' ) ) {
	 ...
	 * 
	 * Output a second part (stored in the previous call of qwery_blog_archive_start).
	 */
...
if ( ! function_exists('qwery_get_the_ID') ) {
	/**
	 * Return current post ID
	 * 
	 * @return int  Current post ID
	 */
	function qwery_get_the_ID() {
		global $wp_query;
		return in_the_loop() 
					? get_the_ID() 
					: ( ! empty( $wp_query->post->ID )
						? $wp_query->post->ID
						: ( qwery_is_singular() && ! empty( $wp_query->queried_object->ID )
...
if ( ! function_exists( 'qwery_is_post_edit' ) ) {
	...
	function qwery_is_post_edit() {
		return ( qwery_check_url( 'post.php' ) && ! empty( $_GET['action'] ) && $_GET['action'] == 'edit')
				...
				||
				( qwery_check_url( '/block-renderer/trx-addons/' ) && ! empty( $_GET['context'] ) && $_GET['context'] == 'edit' )
				||
				( qwery_check_url( 'admin.php' ) && ! empty( $_GET['page'] ) && $_GET['page'] == 'gutenberg-edit-site' )
				||
				( qwery_check_url( 'site-editor.php' ) && ! empty( $_GET['postType'] ) )	// || $_GET['postType'] == 'wp_template' ) )
				||
...
if ( ! function_exists( 'qwery_is_singular' ) ) {
	...
	function qwery_is_singular( $type = '' ) {
		global $wp_query;
		return apply_filters( 'qwery_filter_is_singular', ! empty( $wp_query->queried_object->ID ) && is_singular( $type ), $type );
	}
}
...
if ( ! function_exists( 'qwery_is_single' ) ) {
	...
	function qwery_is_single() {
		global $wp_query;
		return apply_filters( 'qwery_filter_is_single', ! empty( $wp_query->queried_object->ID ) && is_single() );
	}
}
...
if ( ! function_exists( 'qwery_get_nav_menu' ) ) {
	 ...
	 * 									  or true to use a custom walker from the theme with class 'qwery_custom_menu_walker'.
	 * 									  Default is false.
	 *
	 * @return string               A menu layout or an error message.
	 */
	function qwery_get_nav_menu( $location = '', $menu = '', $depth = 0, $custom_walker = false ) {
		static $list = array();
		...
		if ( empty( $list[ $slug ] ) ) {
			...
			if ( $custom_walker ) {
				...
				} else if ( $custom_walker === true && class_exists( 'qwery_custom_menu_walker' ) ) {
...
if ( ! function_exists( 'qwery_query_add_sort_order' ) ) {
	...
	function qwery_query_add_sort_order( $args, $orderby = 'date', $order = 'desc' ) {
		...
		return apply_filters( 'qwery_filter_add_sort_order_args', $args, $orderby, $order );
	}
}
...
if ( ! function_exists( 'qwery_get_content_width' ) ) {
	...
	function qwery_get_content_width() {
		$pg_width = apply_filters( 'qwery_filter_content_width', (int)qwery_get_theme_option( 'page_width' ) );
		$sb_width = 0;
		$sb_gap   = 0;
...
if ( ! function_exists( 'qwery_enqueue_styles' ) ) {
	 ...
	 */
	function qwery_enqueue_styles( $list, $sc ) {
		if ( is_array( $list ) ) {
			foreach( $list as $handle => $data ) {
				$lib_url = qwery_get_file_url( $data['src'] );
				if ( $lib_url ) {
					wp_enqueue_style(
						$handle,
						$lib_url,
						...
						! empty( $data['media'] ) ? qwery_media_for_load_css_responsive( str_replace( '_', '-', $sc ), $data['media'] ) : 'all'
					);
				}
			}
		}
...
if ( ! function_exists( 'qwery_enqueue_scripts' ) ) {
	/**
	 * Enqueue a list of scripts
	 *
	 * @param array $list - scripts to enqueue
	 */
	function qwery_enqueue_scripts( $list, $sc ) {
		if ( is_array( $list ) ) {
			foreach( $list as $handle => $data ) {
				$lib_url = qwery_get_file_url( $data['src'] );
				if ( $lib_url ) {
...
if ( ! function_exists( 'qwery_enqueue_optimized' ) ) {
	/**
	 ...
	 */
	function qwery_enqueue_optimized( $sc, $force, $args ) {
		static $loaded = array();
		if ( empty( $loaded[ $sc ] ) && (
			current_action() == 'wp_enqueue_scripts' && qwery_need_frontend_scripts( ! empty( $args['slug'] ) ? $args['slug'] :  $sc )
			||
			...
			if ( ! isset( $args['need'] ) || $args['need'] ) {
				if ( ! empty( $args['css'] ) ) {
					qwery_enqueue_styles( $args['css'], $sc );
				}
				if ( ! empty( $args['js'] ) ) {
					qwery_enqueue_scripts( $args['js'], $sc );
				}
...
if ( ! function_exists( 'qwery_enqueue_optimized_responsive' ) ) {
	/**
	 ...
	 * @param array $args - arguments with styles and scripts to enqueue
	 */
	function qwery_enqueue_optimized_responsive( $sc, $force, $args ) {
		static $loaded = array();
		if ( empty( $loaded[ $sc ] ) && (
			current_action() == 'wp_enqueue_scripts' && qwery_need_frontend_scripts( ! empty( $args['slug'] ) ? $args['slug'] :  $sc )
			||
...
if ( ! function_exists( 'qwery_get_privacy_text' ) ) {
	...
	function qwery_get_privacy_text() {
		return apply_filters( 'qwery_filter_privacy_text', wp_kses( trim( qwery_get_theme_option( 'privacy_text' ) ), 'qwery_kses_content' ) );
	}
}

if ( ! function_exists( 'qwery_add_privacy_page_link' ) ) {
	/**
	 * Adds the Privacy Policy page link to the Privacy text
	 * 
	 * @hooked filter qwery_filter_privacy_text
	 * 
	 * @param string $text - the text to modify
	 *
	 * @return string  A label text.
	 */
	function qwery_add_privacy_page_link( $text ) {
		if ( ! empty( $text ) ) {
			...
			if ( ! empty( $page ) ) {
				$text .=  ( ! in_array( substr( $text, -1 ), array( '.', '!' ) ) ? '.' : '' )
							...
							. sprintf(
								__( 'For further details on handling user data, see our %s.', 'qwery' ),
								'<a href="' . esc_url( get_permalink( $page ) ) . '" target="_blank">' . __( 'Privacy Policy', 'qwery' ) . '</a>'
							);
			}
		}
		return $text;
...
if ( ! function_exists( 'qwery_kses_allowed_html' ) ) {
	 ...
	 * ( if $context is equal to 'qwery_kses_content' or 'trx_addons_kses_content' ).
	 *
	 * Hooks:
	 *
	 * add_filter( 'wp_kses_allowed_html', 'qwery_kses_allowed_html', 10, 2 );
...
if ( ! function_exists( 'qwery_verify_nonce' ) ) {
	 ...
	 */
	function qwery_verify_nonce( $nonce = 'nonce', $mask = '', $die = true ) {
		$rez = true;
		if ( empty( $mask ) ) {
