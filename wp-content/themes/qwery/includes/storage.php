...
if ( ! function_exists( 'qwery_storage_get_array' ) ) {
	...
	function qwery_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $QWERY_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $QWERY_STORAGE[ $var_name ][ $key ] ) ? $QWERY_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $QWERY_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $QWERY_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}
