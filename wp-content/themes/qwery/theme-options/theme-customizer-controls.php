...
class Qwery_Customize_Theme_Control extends WP_Customize_Control {
	...
	protected function end_render_field() {
		if ( ! empty( $this->input_attrs['data-pro-only'] ) ) {
			qwery_show_layout( qwery_add_inherit_cover( $this->id, array( 'type' => $this->type, 'pro_only' => true ) ) );
		}
		?>
		</div>
		<?php
	...
	protected function render_field_description() {
		if ( ! empty( $this->description ) ) {
			?>
			<span class="customize-control-description description"><?php qwery_show_layout( $this->description ); ?></span>
			<?php
		}
	}
...
// 'info' field
//--------------------------------------------------------------------
class Qwery_Customize_Info_Control extends Qwery_Customize_Theme_Control {
	public $type = 'info';
...
// 'hidden' field
//--------------------------------------------------------------------
class Qwery_Customize_Hidden_Control extends Qwery_Customize_Theme_Control {
	public $type = 'hidden';
	...
	protected function render_content() {
		...
		if ( 'last_option' == $this->id && qwery_storage_get( 'need_footer_scripts', false ) ) {
			qwery_storage_set( 'need_footer_scripts', false );
...
// 'button' field
//--------------------------------------------------------------------
class Qwery_Customize_Button_Control extends Qwery_Customize_Theme_Control {
	public $type = 'button';
...
// 'switch' field
//--------------------------------------------------------------------
class Qwery_Customize_Switch_Control extends Qwery_Customize_Theme_Control {
	public $type = 'switch';
	...
	protected function render_content() {
		...
		$this->render_field_description();
		?>
		<label class="customize-control-field-wrap qwery_options_item_switch">
			<input type="hidden"
				<?php
				$this->link();
				if ( ! empty( $this->input_attrs['var_name'] ) ) {
...
// 'icon' field
//--------------------------------------------------------------------
class Qwery_Customize_Icon_Control extends Qwery_Customize_Theme_Control {
	public $type = 'icon';
...
// 'checklist' field
//--------------------------------------------------------------------
class Qwery_Customize_Checklist_Control extends Qwery_Customize_Theme_Control {
...
// 'choice' field
//--------------------------------------------------------------------
class Qwery_Customize_Choice_Control extends Qwery_Customize_Theme_Control {
	public $type = 'choice';
...
// 'scheme_editor' field
//--------------------------------------------------------------------
class Qwery_Customize_Scheme_Editor_Control extends Qwery_Customize_Theme_Control {
		...
		/>
			...
			qwery_show_layout(
				qwery_show_custom_field(
					...
					$this->input_attrs,
					qwery_unserialize( $this->input_attrs['value'] )
				)
			);
			?>
		</span>
...
// 'text_editor' field
//--------------------------------------------------------------------
class Qwery_Customize_Text_Editor_Control extends Qwery_Customize_Theme_Control {
	...
	protected function render_content() {
		...
		qwery_storage_set( 'need_footer_scripts', true );
	}
}
...
// 'range' field
//--------------------------------------------------------------------
class Qwery_Customize_Range_Control extends Qwery_Customize_Theme_Control {
	public $type = 'range';
	...
	protected function render_content() {
		...
		<span class="customize-control-field-wrap"><input type="<?php echo ! $show_value ? 'hidden' : 'text'; ?>" 
			...
			$this->link();
			if ( $show_value ) {
				echo ' class="qwery_range_slider_value"';
			}
			if ( ! empty( $this->input_attrs['var_name'] ) ) {
				echo ' data-var_name="' . esc_attr( $this->input_attrs['var_name'] ) . '"';
			}
