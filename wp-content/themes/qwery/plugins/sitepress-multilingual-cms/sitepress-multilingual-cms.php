...
if ( ! function_exists( 'qwery_wpml_make_theme_options_translatable' ) ) {
	add_action( 'after_setup_theme', 'qwery_wpml_make_theme_options_translatable', 3 );
	function qwery_wpml_make_theme_options_translatable() {
		if ( qwery_exists_wpml() ) {
			$translatable = apply_filters( 'qwery_filter_theme_options_translatable', array() );
			if ( is_array( $translatable ) ) {
				foreach( $translatable as $option_name ) {
					if ( qwery_storage_isset( 'options', $option_name ) ) {
						qwery_storage_set_array2( 'options', $option_name, 'translate', true );
					}
				}
			}
		}
