...
jQuery( document ).ready(
	function() {
		...
		qwery_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="qwery_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.qwery_post_editor' );
						if ( link.length > 0 ) {
							qwery_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.qwery_post_editor', function(e) {
						qwery_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('qwery_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function qwery_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
