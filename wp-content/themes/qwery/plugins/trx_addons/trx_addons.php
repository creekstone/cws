...
if ( ! function_exists( 'qwery_trx_addons_theme_setup9' ) ) {
	...
	function qwery_trx_addons_theme_setup9() {
		if ( qwery_exists_trx_addons() ) {
			...
			add_filter( 'trx_addons_filter_video_width', 'qwery_trx_addons_video_width', 10, 2 );
			add_filter( 'trx_addons_filter_video_dimensions', 'qwery_trx_addons_video_dimensions', 10, 2 );
			add_filter( 'qwery_filter_video_dimensions', 'qwery_trx_addons_video_dimensions_in_video_list', 10, 2 );
			...
			add_filter( 'trx_addons_filter_no_image', 'qwery_trx_addons_no_image', 10, 2 );
			...
			add_filter( 'trx_addons_filter_get_theme_bg_color', 'qwery_trx_addons_get_theme_bg_color' );
			...
			if ( is_admin() ) {
				...
				add_filter( 'trx_addons_filter_custom_meta_value_strip_tags', 'qwery_trx_addons_custom_meta_value_strip_tags' );
			}
		...
		if ( is_admin() ) {
			...
			add_filter( 'qwery_filter_search_form_url', 'qwery_trx_addons_filter_search_form_url', 10, 1 );
		}
	}
}
...
if ( ! function_exists( 'qwery_trx_addons_get_list_cpt_options' ) ) {
	function qwery_trx_addons_get_list_cpt_options( $cpt ) {
		...
		foreach ( $options as $k => $v ) {
			...
			// Replace class 'trx_addons_column-' with 'qwery_column-'
			$options[ $k ] = qwery_array_str_replace( 'trx_addons_column-', 'qwery_column-', 'class', $options[ $k ] );
		}
		return $options;
	}
}
...
if ( ! function_exists( 'qwery_trx_addons_init_settings' ) ) {
	...
	function qwery_trx_addons_init_settings( $settings ) {
		...
		$settings['icons_source']              = qwery_get_theme_setting( 'icons_source' );
...
if ( ! function_exists( 'qwery_trx_addons_custom_meta_value_strip_tags' ) ) {
	// Handler of the add_filter( 'trx_addons_filter_custom_meta_value_strip_tags', 'qwery_trx_addons_custom_meta_value_strip_tags' );
	function qwery_trx_addons_custom_meta_value_strip_tags( $keys ) {
		return is_array( $keys ) ? qwery_array_delete_by_value( $keys, 'price' ) : $keys;
	}
}
...
if ( ! function_exists( 'qwery_get_custom_layout_id' ) ) {
	function qwery_get_custom_layout_id( $type, $layout_style = '' ) {
		...
		} else if ( defined( 'QWERY_FSE_TEMPLATE_PART_PT' ) && strpos( $layout_style, "{$type}-fse-template-" ) !== false ) {
			$layout_prefix = "{$type}-fse-template-";
			$layout_cpt = defined( 'QWERY_FSE_TEMPLATE_PART_PT' ) ? QWERY_FSE_TEMPLATE_PART_PT : 'wp_template_part';
		}
...
if ( ! function_exists( 'qwery_trx_addons_video_width' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_video_width', 'qwery_trx_addons_video_width', 10, 2 );
	function qwery_trx_addons_video_width( $width, $type = 'theme' ) {
		if ( $type == 'theme' ) {
			$width = qwery_get_content_width();
		}
		return $width;
	}
}
...
if ( ! function_exists( 'qwery_trx_addons_video_dimensions' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_video_dimensions', 'qwery_trx_addons_video_dimensions', 10, 2 );
	function qwery_trx_addons_video_dimensions( $dim, $ratio = '16:9' ) {
		if ( $ratio == '16:9' ) {
			$parts = explode( ':', apply_filters( 'qwery_filter_video_ratio', $ratio ) );
			$dim['width']  = qwery_get_content_width();
			$dim['height'] = round( $dim['width'] / $parts[0] * $parts[1] );
		}
		return apply_filters( 'qwery_filter_video_dimensions', $dim, $ratio );
	}
}
...
if ( ! function_exists( 'qwery_trx_addons_video_dimensions_in_video_list' ) ) {
	//Handler of the add_filter( 'qwery_filter_video_dimensions', 'qwery_trx_addons_video_dimensions_in_video_list', 10, 2 );
	function qwery_trx_addons_video_dimensions_in_video_list( $dim, $ratio = '16:9' ) {
		// If inside "Video list" - reduce dimensions, because controller is present
...
if ( ! function_exists( 'qwery_trx_addons_no_image' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_no_image', 'qwery_trx_addons_no_image', 10, 2);
	function qwery_trx_addons_no_image( $no_image = '', $need = false ) {
		return qwery_get_no_image( $no_image, $need );
	}
}
...
if ( ! function_exists( 'qwery_trx_addons_get_list_icons_classes' ) ) {
	...
	function qwery_trx_addons_get_list_icons_classes( $list, $prepend_inherit ) {
		return in_array( qwery_get_theme_setting( 'icons_source' ), array( 'theme', 'both' ) )
					? qwery_get_list_icons( 'icons', $prepend_inherit )
					: $list;
	}
}
...
if ( ! function_exists( 'qwery_trx_addons_get_theme_bg_color' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_get_theme_bg_color', 'qwery_trx_addons_get_theme_bg_color' );
	function qwery_trx_addons_get_theme_bg_color( $color ) {
		return qwery_get_scheme_color( 'bg_color' );
	}
}
...
if ( ! function_exists( 'qwery_trx_addons_filter_search_form_url' ) ) {
	//Handler of the add_filter( 'qwery_filter_search_form_url', 'qwery_trx_addons_filter_search_form_url', 10, 1 );
	function qwery_trx_addons_filter_search_form_url( $url ) {
...
if ( ! function_exists( 'qwery_trx_addons_action_show_layout' ) ) {
	...
	function qwery_trx_addons_action_show_layout( $layout_id = '', $post_id = 0 ) {
		if ( ! apply_filters( 'qwery_filter_custom_layout_shown', false, $layout_id, $post_id ) ) {
...
if ( qwery_exists_trx_addons() ) {
	$qwery_fdir = qwery_get_file_dir( 'plugins/trx_addons/trx_addons-style.php' );
	if ( ! empty( $qwery_fdir ) ) {
