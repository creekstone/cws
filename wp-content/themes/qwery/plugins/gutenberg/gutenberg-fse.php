...
if ( ! defined( 'QWERY_FSE_TEMPLATE_PART_PT' ) ) define( 'QWERY_FSE_TEMPLATE_PART_PT', 'wp_template_part' );

if ( ! defined( 'QWERY_FSE_TEMPLATE_PART_AREA_HEADER' ) )  define( 'QWERY_FSE_TEMPLATE_PART_AREA_HEADER',  defined( 'WP_TEMPLATE_PART_AREA_HEADER' )  ? WP_TEMPLATE_PART_AREA_HEADER  : 'header' );
if ( ! defined( 'QWERY_FSE_TEMPLATE_PART_AREA_FOOTER' ) )  define( 'QWERY_FSE_TEMPLATE_PART_AREA_FOOTER',  defined( 'WP_TEMPLATE_PART_AREA_FOOTER' )  ? WP_TEMPLATE_PART_AREA_FOOTER  : 'footer' );
if ( ! defined( 'QWERY_FSE_TEMPLATE_PART_AREA_SIDEBAR' ) ) define( 'QWERY_FSE_TEMPLATE_PART_AREA_SIDEBAR', defined( 'WP_TEMPLATE_PART_AREA_SIDEBAR' ) ? WP_TEMPLATE_PART_AREA_SIDEBAR : 'sidebar' );
if ( ! defined( 'QWERY_FSE_TEMPLATE_PART_AREA_UNCATEGORIZED' ) ) define( 'QWERY_FSE_TEMPLATE_PART_AREA_UNCATEGORIZED', defined( 'WP_TEMPLATE_PART_AREA_UNCATEGORIZED' ) ? WP_TEMPLATE_PART_AREA_UNCATEGORIZED : 'uncategorized' );

// Load additional files with FSE support
require_once qwery_get_file_dir( 'plugins/gutenberg/gutenberg-fse-lists.php' );
require_once qwery_get_file_dir( 'plugins/gutenberg/gutenberg-fse-templates.php' );
...
if ( ! function_exists( 'qwery_gutenberg_is_fse_wp_support' ) ) {
	/**
	 * Check if WordPress with FSE support installed
	 * (WordPress 5.9+ Site Editor is present).
	 */
	function qwery_gutenberg_is_fse_wp_support() {
		return function_exists( 'wp_is_block_theme' );
	}
}

if ( ! function_exists( 'qwery_gutenberg_is_fse_gb_support' ) ) {
	/**
	 * Check if a plugin "Gutenberg" with FSE support is installed.
	 */
	function qwery_gutenberg_is_fse_gb_support() {
...
if ( ! function_exists( 'qwery_exists_gutenberg_fse' ) ) {
	...
	function qwery_exists_gutenberg_fse() {
		return qwery_exists_gutenberg() && ( qwery_gutenberg_is_fse_wp_support() || qwery_gutenberg_is_fse_gb_support() );
	}
}
...
if ( ! function_exists( 'qwery_gutenberg_is_fse_theme' ) ) {
	...
	function qwery_gutenberg_is_fse_theme() {
		static $fse = -1;
		if ( $fse == -1 ) {
			if ( qwery_gutenberg_is_fse_wp_support() ) {                                    // WordPress 5.9+ FSE
				$fse = (bool) wp_is_block_theme();
			} else if ( qwery_gutenberg_is_fse_gb_support() ) {                             // Plugin Gutenberg FSE
				$fse = (bool) gutenberg_is_fse_theme();
			} else {
				$fse = is_readable( QWERY_THEME_DIR . 'templates/index.html' )              // WordPress 5.9+ FSE
						|| ( QWERY_THEME_DIR != QWERY_CHILD_DIR && is_readable( QWERY_CHILD_DIR . 'templates/index.html' ) )
						|| is_readable( QWERY_THEME_DIR . 'block-templates/index.html' )    // Plugin Gutenberg FSE
						|| ( QWERY_THEME_DIR != QWERY_CHILD_DIR && is_readable( QWERY_CHILD_DIR . 'block-templates/index.html' ) );
			}
		}
		return $fse;
	}
...
if ( ! function_exists( 'qwery_gutenberg_fse_skin_updated' ) ) {
	add_action( 'qwery_action_theme_updated', 'qwery_gutenberg_fse_skin_updated' );
	add_action( 'qwery_action_skin_updated', 'qwery_gutenberg_fse_skin_updated', 10, 1 );
	/**
	 * Copy FSE-required folders from an updated skin's subfolder '_fse' to the theme's root folder.
	 * 
	 * Hooks: add_action( 'qwery_action_skin_updated', 'qwery_gutenberg_fse_skin_updated', 10, 1 );
	 */
	function qwery_gutenberg_fse_skin_updated( $skin = '' ) {
		// Get an active skin name if empty
		if ( current_action() == 'qwery_action_theme_updated' && empty( $skin ) ) {
			$skin = qwery_skins_get_active_skin_name();
		}
		if ( qwery_skins_get_current_skin_name() == $skin ) {
			qwery_gutenberg_fse_copy_from_skin( $skin );
			qwery_gutenberg_fse_update_theme_json( $skin );
		}
	}
}
...
if ( ! function_exists( 'qwery_gutenberg_fse_copy_from_skin' ) ) {
	 ...
	 * 
	 * Hooks: add_action( 'qwery_action_skin_switched', 'qwery_gutenberg_fse_copy_from_skin', 10, 2 );
	 */
	function qwery_gutenberg_fse_copy_from_skin( $new_skin, $old_skin = '' ) {
		$theme_templates_dir = qwery_prepare_path( QWERY_THEME_DIR . ( qwery_gutenberg_is_fse_wp_support() ? 'templates/' : 'block-templates/' ) );
		$theme_template_parts_dir = qwery_prepare_path( QWERY_THEME_DIR . ( qwery_gutenberg_is_fse_wp_support() ? 'parts/' : 'block-template-parts/' ) );
		$theme_json = qwery_prepare_path( QWERY_THEME_DIR . 'theme.json' );
		$skin_templates_dir = qwery_prepare_path( QWERY_THEME_DIR . 'skins/' . $new_skin . '/_fse' . ( qwery_gutenberg_is_fse_wp_support() ? '/templates/' : '/block-templates/' ) );
		$skin_template_parts_dir = qwery_prepare_path( QWERY_THEME_DIR . 'skins/' . $new_skin . '/_fse' . ( qwery_gutenberg_is_fse_wp_support() ? '/parts/' : '/block-template-parts/' ) );
		...
		if ( is_dir( $skin_templates_dir ) ) {
			qwery_copy( $skin_templates_dir, $theme_templates_dir );
		}
		if ( is_dir( $skin_template_parts_dir ) ) {
			qwery_copy( $skin_template_parts_dir, $theme_template_parts_dir );
		}
	}
}

if ( ! function_exists( 'qwery_gutenberg_fse_update_theme_json' ) ) {
	add_action( 'qwery_action_skin_switched', 'qwery_gutenberg_fse_update_theme_json', 30, 2 );
	add_action( 'qwery_action_save_options', 'qwery_gutenberg_fse_update_theme_json', 30 );
	add_action( 'trx_addons_action_save_options', 'qwery_gutenberg_fse_update_theme_json', 30 );
	/**
	 * Update the file 'theme.json' after the current skin is switched and/or theme options are saved.
	 * 
	 * Hooks: add_action( 'qwery_action_skin_switched', 'qwery_gutenberg_fse_update_theme_json', 30, 2 );
	 * 
	 *        add_action( 'qwery_action_save_options', 'qwery_gutenberg_fse_update_theme_json', 30 );
	 * 
	 *        add_action( 'trx_addons_action_save_options', 'qwery_gutenberg_fse_update_theme_json', 30 );
	 * 
	 * Trigger filter 'qwery_filter_gutenberg_fse_theme_json_data' to allow other modules modify this data before saving.
	 */
	function qwery_gutenberg_fse_update_theme_json( $new_skin = '', $old_skin = '' ) {
		// Get an active skin name if empty
		if ( in_array( current_action(), array( 'qwery_action_save_options', 'trx_addons_action_save_options' ) ) && empty( $new_skin ) ) {
			$new_skin = qwery_skins_get_active_skin_name();
		}
		// If the current skin supports FSE
		if ( ! empty( $new_skin ) ) {	// && is_dir( QWERY_THEME_DIR . qwery_skins_get_current_skin_dir( $new_skin ) . '_fse' ) ) {
			$theme_json_data = qwery_gutenberg_fse_theme_json_data();
			if ( ! empty( $theme_json_data ) && is_array( $theme_json_data ) ) {
				qwery_fpc( qwery_prepare_path( QWERY_THEME_DIR . 'theme.json' ), json_encode( $theme_json_data, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES ) );
			}
		}
	}
}

if ( ! function_exists( 'qwery_gutenberg_fse_theme_json_data' ) ) {
	/**
	 * Return a default data to save to the file 'theme.json'
	 * 
	 * Trigger filter 'qwery_filter_gutenberg_fse_theme_json_data' to allow other modules change data.
	 * 
	 * @return array  Data for the 'theme.json'
	 */
	function qwery_gutenberg_fse_theme_json_data() {
...
*/
		...
		if ( ! empty( $scheme[ qwery_get_scheme_color_name( 'bg_color' ) ] ) && ! empty( $scheme[ qwery_get_scheme_color_name( 'text' ) ] ) ) {
			$data['settings']['color']['duotone'][] = array(
				"colors" => array( $scheme[ qwery_get_scheme_color_name( 'bg_color' ) ], $scheme[ qwery_get_scheme_color_name( 'text' ) ] ),
				"slug"   => 'bg-and-text',
				"name"   => esc_html__( 'Background and text color', 'qwery' )
			);
		}
		if ( ! empty( $scheme[ qwery_get_scheme_color_name( 'bg_color' ) ] ) && ! empty( $scheme[ qwery_get_scheme_color_name( 'text_dark' ) ] ) ) {
			$data['settings']['color']['duotone'][] = array(
				"colors" => array( $scheme[ qwery_get_scheme_color_name( 'bg_color' ) ], $scheme[ qwery_get_scheme_color_name( 'text_dark' ) ] ),
				"slug"   => 'bg-and-dark',
				"name"   => esc_html__( 'Background and dark color', 'qwery' )
			);
		}
		if ( ! empty( $scheme[ qwery_get_scheme_color_name( 'bg_color' ) ] ) && ! empty( $scheme[ qwery_get_scheme_color_name( 'text_link' ) ] ) ) {
			$data['settings']['color']['duotone'][] = array(
				"colors" => array( $scheme[ qwery_get_scheme_color_name( 'bg_color' ) ], $scheme[ qwery_get_scheme_color_name( 'text_link' ) ] ),
				"slug"   => 'bg-and-link',
				"name"   => esc_html__( 'Background and link color', 'qwery' )
			);
		}

		// Add gradients
		if ( ! empty( $scheme[ qwery_get_scheme_color_name( 'text_hover' ) ] ) && ! empty( $scheme[ qwery_get_scheme_color_name( 'text_link' ) ] ) ) {
			$data['settings']['color']['gradients'][] = array(
				...
				"name"     => esc_html__( 'Vertical from link color to hover color', 'qwery' ),
			);
			$data['settings']['color']['gradients'][] = array(
				...
				"name"     => esc_html__( 'Diagonal from link color to hover color', 'qwery' ),
			);
		}
...
*/
		...
		if ( qwery_gutenberg_is_fse_theme() ) {
			...
			$data['customTemplates'] = array(
				array(
					"name"      => "blog",
					"title"     => esc_html__( "Blog archive", 'qwery' ),
					"postTypes" => array(
						"page"
					)
				)
			...
			$data['templateParts'] = array(
				array(
					"name"  => "header",
					"title" => esc_html__( "Header for FSE", 'qwery' ),
					"area"  => QWERY_FSE_TEMPLATE_PART_AREA_HEADER,
				),
				array(
					"name"  => "sidebar",
					"title" => esc_html__( "Sidebar for FSE", 'qwery' ),
					"area"  => QWERY_FSE_TEMPLATE_PART_AREA_SIDEBAR,
				),
				array(
					"name"  => "content-404",
					"title" => esc_html__( "Page 404 content", 'qwery' ),
					"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
				),
				array(
					"name"  => "none-archive",
					"title" => esc_html__( "No posts found", 'qwery' ),
					"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
				),
				array(
					"name"  => "none-search",
					"title" => esc_html__( "Nothing matched search", 'qwery' ),
					"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
				),
				array(
					"name"  => "blog-post-standard",
					"title" => esc_html__( "Blog post item: Standard", 'qwery' ),
					"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
				),
				array(
					"name"  => "blog-post-header",
					"title" => esc_html__( "Blog post header", 'qwery' ),
					"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
				),
				array(
					"name"  => "blog-pagination",
					"title" => esc_html__( "Blog pagination", 'qwery' ),
					"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
				),
				array(
					"name"  => "footer",
					"title" => esc_html__( "Footer for FSE", 'qwery' ),
					"area"  => WP_TEMPLATE_PART_AREA_FOOTER,
				),
			);
		}

		return apply_filters( 'qwery_filter_gutenberg_fse_theme_json_data', $data );
	}
}
