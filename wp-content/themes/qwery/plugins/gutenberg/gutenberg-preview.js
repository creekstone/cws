...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			qwery_create_observer( 'qwery-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				qwery_gutenberg_editor_init();
			} );
		} else {
		 	qwery_gutenberg_editor_init();
		}
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'qwery_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'qwery_inited' ) ) {
	...
	// Init on page load
	function qwery_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = qwery_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + QWERY_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + QWERY_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in QWERY_STORAGE['override_classes'] ) {
				$obj.addClass( QWERY_STORAGE['override_classes'][i].replace( '%s', QWERY_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( qwery_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && QWERY_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'qwery_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			qwery_remove_observer( 'qwery-check-editor-styles-wrapper' );
			qwery_create_observer( 'qwery-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'qwery_inited' );
	}
} );
